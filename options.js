...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (settings["tempUnit"]) {
    document.querySelector(
      `input[name="temp-unit"][value="${settings.tempUnit}"]`,
    ).checked = true;
  }
  ...
  if (settings["sidebarShowCustomize"] === undefined)
    settings["sidebarShowCustomize"] = true;
  const sidebarShowCustomizeCheckbox = document.getElementById(
    "sidebar-show-customize",
  );
    ...
    if (sidebarShowCustomizeCheckbox) {
      if (expanded) {
        ...
        const wrapper = document.getElementById(
          "sidebar-show-customize-wrapper",
        );
  ...
  saveBtn.addEventListener("click", () => {
    ...
    settings_keys.map((key) => {
      if (key == "topRightOrder") {
        ...
        if (
          settings_obj[key] !== settings.useCustomCity ||
          settings_obj.customCity !== settings.customCity
        ) {
          localStorage.removeItem("weatherData");
        }
        ...
        widgetRows.forEach((row) => {
          selectedWidgets.push(
            row.querySelector("input").getAttribute("data-widget"),
          );
        });
  ...
  const sidebarExpandedCheck = document.getElementById(
    "sidebar-expanded-check",
  );
...
document.getElementById("restore-defaults").addEventListener("click", () => {
  ...
  showNotification(
    "Settings restored to defaults! Reloading...",
    2000,
    "restore",
    true,
  );
